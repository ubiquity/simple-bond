...
task("abi", "Get contract abi")
  ...
  .setAction(async (taskArgs: { nameOrAddress: string }, { deployments, ethers, getChainId, network }) => {
    ...
    const chainId1 = Number(await getChainId());
    console.log("network", chainId1, network.name, network.live);

    const { chainId, name: chainName } = await ethers.provider.getNetwork();
    console.log("network", chainId, chainName);
